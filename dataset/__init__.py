...
def get_path_dataset(
    config, path_dict, length=None, test=False, batch_size=None, get_loader=False
):
...
def get_dataset(
    ...
    if class_wise:
        ...
        dataset = VideoDataset(
            ...
            epic_kitchens=config.epic_kitchens,
            remove_target_private=config.remove_target_private,
            n_classes=class_set,
            # root_path,
            # list_path,
            # transforms,
...
def init_pair_dataset(
    ...
    src_loader = get_dataset(
        config, config.source, config.source_classes, length=length
    )
    tgt_loader = get_dataset(
        config,
        config.target,
...
def init_class_dataset(
    config, plabel_dict, src_class_set, tgt_class_set, length=None, uk_list=None
):
...
def init_target_dataset(
    config, plabel_dict, tgt_class_set, length=None, uk_list=None, binary_label=None
):
    transform = get_transform()
    if config.task == "visda" or config.task == "imagenet-caltech":
        root_path = osp.join(config.root[config.task], config.source)
    ...
    if uk_list is not None:
        tgt_class_set.append(config.num_classes)
    dataset = TargetClassAwareDataset(
        root_path,
        config.num_pclass,
        transform,
        tgt_class_set,
