...
class Trainer(BaseTrainer):
    def iter(self, i_iter):
        ...
        s_video, s_label = s_batch
        s_video = s_video.cuda()
        s_label = s_label.cuda()
        #_, s_neck, s_prob, _ = self.model(s_img)
        b, n_clips, *_ = s_video.size()

        # consider clip as extra video in the batch
        s_video = rearrange(s_video, "b n f c h w -> (b n) f c h w")
        clip_feats = self.model(s_video)
        clip_feats = clip_feats.view(b, n_clips, -1)
        s_feats = self.clip_aggregator(clip_feats)
        s_prob = nn.Linear(self.feat_dim, self.config.cls_share)(s_feats)
    ...
    def train(self):
        ...
        self.clip_aggregator = MLPAggregation(
            ...
            n_clips=self.config.n_clips,
            layers=self.config.mlp_aggregator_n_layers,
            add_bn=self.config.mlp_aggregator_add_bn,
            dropout=self.config.mlp_aggregator_dropout,
        )
        ...
        self.optimizer = optim.SGD(
            self.model.optim_parameters(self.config.lr),
            lr=self.config.lr,
        ...
        if not self.warmup:
            ...
            self.loader = init_class_dataset(
                self.config,
                plabel_dict=plabel_dict,
                src_class_set=class_set,
                tgt_class_set=class_set,
                length=self.config.stage_size,
            )
            self.s_loader, self.t_loader = init_pair_dataset(
                self.config,
                length=self.config.stage_size,
                plabel_dict=self.cluster_label,
                binary_label=None,
            )
            self.t_loader = init_target_dataset(
                self.config,
                length=self.config.stage_size,
                plabel_dict=self.cluster_label,
    ...
    def consensus_score(
        self, t_feats, t_codes, t_centers, s_feats, s_labels, s_centers, step
    ):
        ...
        if search:
            while search and n_center <= max_center:
                ...
                (
                    ...
                    t_feats, t_codes, t_centers, s_feats, s_labels, s_centers, step
                )
                ...
                if self.config.drop_stop and self.detect_continuous_drop(
                    score_his, n=self.config.drop, con=self.config.drop_con
                ):
            ...
            t_centers, t_codes, _ = self.sklearn_kmeans(
                t_feats, self.config.num_centers, init=self.memory.memory.cpu().numpy()
            )
            st_score, t_score, inter, scores, sub_dict = self.consensus_score(
                t_feats, t_codes, t_centers, s_feats, s_labels, s_centers, step
            )
    ...
    def cluster_matching(self, step):
        ...
        (
            ...
            inter,
            sub_scores,
        ) = self.get_tgt_centers(step, s_centers, s_feats, s_labels)
        gt_vector = torch.stack(list(gt_dict.values()))
        s_centers = F.normalize(s_centers, p=2, dim=-1)
        t_centers = F.normalize(t_centers, p=2, dim=-1)

        filtered_cluster_label, filtered_pair, freq = self.target_filtering(
            t_feats, t_codes, gt_dict, s_feats, s_labels, s_centers, t_centers, inter
        )
        self.clus_acc(
            t_codes.squeeze(), gt_vector.squeeze(), inter, gt_freq, sub_scores
        )
        ...
        self.private_label_set = [
            i for i in range(self.config.num_classes) if i not in label_set
        ]
        self.private_mapping = {
            self.private_label_set[i]: i for i in range(len(self.private_label_set))
        }
