...
class BaseTrainer(object):
    def __init__(self, config, writer):
        ...
        if self.config.task == "imagenet-caltech":
            self.src_loader = get_cls_sep_dataset(
                self.config,
                self.config.source,
    ...
    def open_validate(self, i_iter):
        ...
        )  # mapping between source label and target cluster index
        ...
        gt_all = torch.where(
            gt_all >= self.config.num_classes,
            torch.Tensor([uk_index]).cuda(),
            gt_all.float(),
        )
        ...
        self.neptune_metric("memo-val/Test Accuracy[center]", acc)
        self.neptune_metric(
            "memo-val/UK classification accuracy[center]", accs.avg[uk_index]
        )
    ...
    def get_src_centers(self):
        ...
        if self.config.model != "res50":
            if self.config.model == "i3d":
                with torch.no_grad():
                    x = torch.zeros((2, 3, 16, 224, 224))
                    out = self.model(x)
                feat_dim = out.size(1)
                s_center = torch.zeros((num_cls, feat_dim)).float().cuda()
            else:
        ...
        for _, batch in tqdm(enumerate(self.src_loader)):
            ...
            with torch.no_grad():
                feat = self.model(img.cuda())
            feat = F.normalize(feat, p=2, dim=-1)
            N, C = feat.shape
            s_labels.extend(label.tolist())
            s_feats.extend(torch.chunk(feat, N, dim=0))
    ...
    def sklearn_kmeans(self, feat, num_centers, init=None):
        ...
        if init is not None:
            kmeans = KMeans(n_clusters=num_centers, init=init, random_state=0).fit(
                feat.cpu().numpy()
            )
        else:
            kmeans = KMeans(n_clusters=num_centers, random_state=0).fit(
                feat.cpu().numpy()
            )
